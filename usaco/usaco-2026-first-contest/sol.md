...
题解：假设至少需要额外获得 x 个硬币，由于 x 个硬币类型随机，我们要考虑最坏情况。我们发现很难直接用上面的字母表达出 x，不妨先假定一个 x，然后判断 x 是否能在最坏情况下换取足够的 A 类硬币。如果某一个 x 可行，那么大于它的 x 必然可行，所以我们可以用 x 来二分答案。
...
二分答案的条件就是，在最差情况下总共获得 A 硬币大于等于 fA 个。
